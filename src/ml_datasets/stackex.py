from river.datasets import base
import stream


class Stackex(base.RemoteDataset):
    """
    It is a collection of six datasets generated from the text collected in a selection of Stack Exchange forums. It includes stackex_chess, stackex_chemistry, stackex_coffee, stackex_cooking, stackex_cs and stackex_philosophy.

    Francisco Charte and David Charte. Working with multilabel datasets in R: The mldr package. The R Journal, 7(2):149-162, 2015.
    """
    def __init__(self):
        super().__init__(
            task=base.MO_BINARY_CLF,
            n_samples=1675,
            n_features=585,
            n_outputs=227,
            url="http://www.uco.es/grupos/kdis/MLLResources/ucobigfiles/Datasets/Full/Stackex_chess_Meka.zip",
            unpack=True,
            filename="Stackex_chess.arff",
            size=2_742_442,
        )

    def _iter(self):
        return stream.iter_arff(
            self.path,
            sparse=False,
            target=[
                "tag_1.d4",
                "tag_1.e4",
                "tag_50-move-rule",
                "tag_advantage",
                "tag_aggressive-play",
                "tag_alekhine",
                "tag_alekhines-defense",
                "tag_analysis",
                "tag_anand",
                "tag_android",
                "tag_annotation",
                "tag_arena",
                "tag_aronian",
                "tag_attack",
                "tag_babaschess",
                "tag_bad-bishop",
                "tag_beginner",
                "tag_berlin",
                "tag_best-practice",
                "tag_big-list",
                "tag_birds-opening",
                "tag_bishop-endgame",
                "tag_bishop-pair",
                "tag_bishops",
                "tag_blindfold-chess",
                "tag_blitz",
                "tag_blunder",
                "tag_board-vision",
                "tag_books",
                "tag_brute-force",
                "tag_bughouse",
                "tag_bullet-chess",
                "tag_calculation",
                "tag_candidate-move",
                "tag_capablanca",
                "tag_capablanca-chess",
                "tag_captures",
                "tag_carlsen",
                "tag_carlsen-anand-2014",
                "tag_caro-kann",
                "tag_castling",
                "tag_catalan",
                "tag_center",
                "tag_cheating",
                "tag_check",
                "tag_checkmate",
                "tag_chess-algorithms",
                "tag_chess-blog",
                "tag_chess-clocks",
                "tag_chess-variants",
                "tag_chess960",
                "tag_chessbase",
                "tag_chessboards",
                "tag_claim-draw",
                "tag_closed-position",
                "tag_clubs",
                "tag_colle-system",
                "tag_colle-zukertort",
                "tag_combinations",
                "tag_composing",
                "tag_computer-chess",
                "tag_correspondence-chess",
                "tag_database",
                "tag_defense",
                "tag_doubled-pawn",
                "tag_download-games",
                "tag_draw",
                "tag_dutch-defense",
                "tag_eco",
                "tag_education",
                "tag_elementary-mates",
                "tag_elo",
                "tag_en-passant",
                "tag_endgame",
                "tag_engines",
                "tag_english-opening",
                "tag_equipment",
                "tag_etiquette",
                "tag_evaluation",
                "tag_evans-gambit",
                "tag_famous-games",
                "tag_famous-moves",
                "tag_famous-players",
                "tag_fen",
                "tag_fianchetto",
                "tag_fiction",
                "tag_fide",
                "tag_fischer",
                "tag_forced-move",
                "tag_formation",
                "tag_four-knights",
                "tag_french-defense",
                "tag_fried-liver-attack",
                "tag_fritz",
                "tag_gambits",
                "tag_game-complexity",
                "tag_game-length",
                "tag_gelfand",
                "tag_glicko",
                "tag_grandmaster",
                "tag_grunfeld-defense",
                "tag_handicapping",
                "tag_history",
                "tag_houdini",
                "tag_human-versus-machine",
                "tag_illegal-move",
                "tag_isolated-pawn",
                "tag_italian-game",
                "tag_karjakin",
                "tag_karpov",
                "tag_kasparov",
                "tag_kings",
                "tag_kings-gambit",
                "tag_kings-indian",
                "tag_kingside",
                "tag_knights",
                "tag_kramnik",
                "tag_learning",
                "tag_linux",
                "tag_london-system",
                "tag_losing",
                "tag_mac",
                "tag_master-games",
                "tag_material-balance",
                "tag_mathematics",
                "tag_memorization",
                "tag_middlegame",
                "tag_minor-pieces",
                "tag_minor-variations",
                "tag_modern-benoni",
                "tag_modern-defense",
                "tag_morphy",
                "tag_najdorf",
                "tag_nalimov",
                "tag_nimzo-indian",
                "tag_notation",
                "tag_novelties",
                "tag_novelty",
                "tag_old-indian",
                "tag_online",
                "tag_online-blitz",
                "tag_online-chess",
                "tag_open-source",
                "tag_opening",
                "tag_otb-chess",
                "tag_passed-pawn",
                "tag_patterns",
                "tag_pawn-endgame",
                "tag_pawn-promotion",
                "tag_pawn-structure",
                "tag_pawns",
                "tag_performance",
                "tag_petroff-defense",
                "tag_pgn",
                "tag_pieces",
                "tag_pins",
                "tag_pirc-defense",
                "tag_ply",
                "tag_point-value",
                "tag_popularity",
                "tag_positional-play",
                "tag_preparation",
                "tag_problems",
                "tag_programming",
                "tag_psychology",
                "tag_puzzles",
                "tag_quality",
                "tag_queens",
                "tag_queens-gambit",
                "tag_queens-gambit-accepted",
                "tag_queens-indian",
                "tag_rapid",
                "tag_rating",
                "tag_refuted",
                "tag_retrograde-analysis",
                "tag_rook-and-minor-endgame",
                "tag_rook-endgame",
                "tag_rooks",
                "tag_rules",
                "tag_ruy-lopez",
                "tag_sacrifice",
                "tag_scandinavian-defense",
                "tag_scheveningen",
                "tag_scholastic-chess",
                "tag_scid",
                "tag_scotch-game",
                "tag_shogi",
                "tag_sicilian-defense",
                "tag_simultaneous",
                "tag_slav-defense",
                "tag_smith-morra-gambit",
                "tag_software",
                "tag_spassky",
                "tag_stalemate",
                "tag_statistics",
                "tag_stockfish",
                "tag_stonewall-attack",
                "tag_strategy",
                "tag_studies",
                "tag_study",
                "tag_style",
                "tag_super-grandmaster",
                "tag_tablebases",
                "tag_tactics",
                "tag_tal",
                "tag_talent",
                "tag_teaching",
                "tag_team",
                "tag_terminology",
                "tag_theory",
                "tag_time-control",
                "tag_time-management",
                "tag_titles",
                "tag_tournament",
                "tag_tournament-directors",
                "tag_training",
                "tag_transpositions",
                "tag_two-knights",
                "tag_uci",
                "tag_untagged",
                "tag_uscf",
                "tag_websites",
                "tag_winning",
                "tag_world-championship",
                "tag_xiangqi",
                "tag_zugzwang",
                "tag_zwischenzug",
            ],
        )