...
class GLM:
    ...
    def _fit(self, x, y, sample_weight, get_grad):
        # Some optimizers need to do something before a prediction is made
        self.optimizer.look_ahead(w=self._weights)

        # Calculate the gradient
        gradient, loss_gradient = get_grad(x, y, sample_weight)
    ...
    def _eval_gradient_one(self, x: dict, y: float, sample_weight: float) -> tuple[dict, float]:
    ...
    def learn_one(self, x, y, sample_weight=1.0):
        with self._learn_mode(x):
            return self._fit(x, y, sample_weight, get_grad=self._eval_gradient_one)
    ...
    def _eval_gradient_many(
        self, X: pd.DataFrame, y: pd.Series, samples_weight: float | pd.Series
    ) -> tuple[dict, float]:
        loss_gradient = self.loss.gradient(y_true=y.values, y_pred=self._raw_dot_many(X))
        loss_gradient *= samples_weight
    ...
    def learn_many(self, X: pd.DataFrame, y: pd.Series, samples_weight: float | pd.Series = 1):
        self._y_name = y.name
        with self._learn_mode(set(X)):
            return self._fit(X, y, samples_weight, get_grad=self._eval_gradient_many)
